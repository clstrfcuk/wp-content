...
if ( ! class_exists( 'TOMB_Image_Field' ) ) {

	class TOMB_Image_Field extends TOMB_Fields {
		...
		static function html( $meta, $field ) {
			...
			$show = null;
			if(!empty($meta)) {
				$img  = 'style="background-image: url('.$meta.')"';
				$show = 'show';
			}
			
			// Image url container
			$output  = '<div class="tomb-img-field screenshot '.$show.'" '.$img.'></div>';	
			$output .= '<input type="text" class="tomb-image tomb-image-url '.$show.'" name="'.$field['id'].'" id="'.$field['id'].'" size="70" value="'.$meta.'"/>';
			$output .= '<div class="tomb-image-clearfix"></div>';
			$output .= '<a href="#" class="tomb-open-media tomb-button button-primary" data-frame-title="'.$field['frame_title'].'" data-frame-button="'.$field['frame_button'].'">'.$field['button_upload'].'</a>';
