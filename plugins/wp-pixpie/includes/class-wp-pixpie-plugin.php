...
class WP_Pixpie_Plugin {
  ...
  function wppp_filter_wp_calculate_image_srcset_meta ($image_meta, $size_array, $image_src, $attachment_id){
  	$original_sizes = $image_meta['sizes'];
  	$new_sizes = array();
	foreach ( $original_sizes as $size_name => $size_val ) {
		if ( ! wppp_ends_with( $size_name, SIZE_UNCOMP ) ) {
			$new_sizes[ $size_name ] = $size_val;
		}
	}
	...
	private function define_public_hooks() {
		...
		add_filter(
			...
			array( $this, 'wppp_filter_wp_calculate_image_srcset_meta' ),
			1, 
			4 );
