(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.CS_huebert = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
window.Huebert = require( '../vendor/huebert' );
window.Huebert.localize( csHuebert );
...
( function( factory ) {
  ...
  Huebert.ColorPicker = function( $el, options ) {
    ...
    function update( options ) {
      ...
      if ( options.controls ) {
        updateColorFromControls( options.controls.update );
        updateControls( options.controls.precise );
        updateInputFromColor();
        blankCheck();
      } else {
        if ( options.input ) {
          color = options.input;
          hueSelection = color.toHsl().h;
          blankCheck();
        } else {
          updateInputFromColor();
        }
      ...
      $swatch.find( '.huebert-swatch-output-color' ).css({ backgroundColor: color.toHslString() });

      $spectrum.css({ backgroundColor: tinycolor({ h: hueSelection, s: 1, l: .5 }).toString() });

      $z.find( '.huebert-picker-control-slider-z-value' )
        .removeAttr( 'style' )
        .attr( 'style', 'background-image: ' + linearGradient( getHueWheel( 0 ) ) + ';' );

      var copy = color.clone();

      $a.find( '.huebert-picker-control-slider-a-value' )
        .removeAttr( 'style' )
        .attr( 'style', 'background-image: ' + linearGradient( [copy.setAlpha( 0 ).toRgbString(), copy.setAlpha( 1 ).toRgbString()] ) + ';' );

      if ( false !== options.updateOriginal ) {
        $el.val( formatForOutput() );
        $el.trigger( 'change', { huebert: true } );
      }

      $swatch.toggleClass( 'huebert-swatch-dark', color.getLuminance() < .13 );

    }

    function formatForOutput() {
      if ( 'hsl' == options.outputFormat ) return color.toHslString();
      if ( 'hsv' == options.outputFormat ) return color.toHslString();
      if ( 'hex' == options.outputFormat ) return color.toHexString();
      if ( 'rgb' == options.outputFormat ) return color.toRgbString();
      return color.toString();
    }

    function blankCheck() {
      if ( ! blank ) return;
      if ( previousColor.getAlpha() == color.getAlpha() ) {
        color.setAlpha( 1 );
        updateControlsFromColor();
      }
      blank = false;
    ...
    function updateInputFromColor() {
      $input.val( color.toRgbString() );
    }

    function updateControlsFromColor() {

      var current = color.toHsl();
      var l = current.l * 2;
      current.l = l > 1 ? 1 : l;
      current.h /= 360;
      ...
      for ( var d in options.map ) {
        var v = options.map[d];
        if ( v in current )
          mapped[d] = ( ['s', 'a'].indexOf( v ) == -1 ) ? 1 - current[v] : current[v];
      }

      for ( var prop in mapped ) {
        mapped[prop] = ( mapped[prop] * 100 ).toFixed( 5 ) + '%';
      }

      updateControls( mapped );

    }

    function updateColorFromControls( values ) {

      var mapped = {};

      for ( var d in options.map ) {
        if ( d in values ) {
          mapped[options.map[d]] =
            ( ['s', 'a'].indexOf( options.map[d] ) == -1 ) ? 1 - values[d] : values[d];
        }
      }

      if ( 'h' in mapped ) {
        hueSelection = mapped.h * 360;
      } else {
        mapped.h = hueSelection;
      }
      ...
      if ( 'l' in mapped ) {
        updated.l = updated.l * ( 1 - updated.s * .5 );
      }

      color = tinycolor.fromRatio( updated );

    }

    function updateControls( offsets ) {

      if ( 'x' in offsets ) {
        $iris.css( { left: offsets.x } );
      }

      if ( 'y' in offsets  ) {
        $iris.css({ top: offsets.y });
      }

      if ( 'z' in offsets  ) {
        $z.find( '.huebert-picker-control-slider-handle' ).css({ left: offsets.z });
      }

      if ( 'a' in offsets ) {
        $a.find( '.huebert-picker-control-slider-handle' ).css({ left: offsets.a });
      }
    ...
    function originalInputChange( e, params ) {
      ...
      if ( newColor.isValid() )
        update( { input: newColor, updateOriginal: false } );

    }

    function clearValue() {
      if ( blank ) return;
      update( { reset: tinycolor( transparent ), updateOriginal: false } );
      $el.val( '' );
      $input.val( '' );
      blank = true;
      $el.trigger( 'change', { huebert: true });
    ...
    function revertValue() {
      var defaultValue = ( options.defaultValue ) ?
        options.defaultValue : $el.attr( 'data-huebert-default-value' ) || transparent;
      update( { reset: tinycolor( defaultValue ) } );
    }
    ...
    function controlClick( e ) {
      ...
      var controlDimensions = {
        x: { pos: $spectrum.offset().left, len: $spectrum.width() },
        y: { pos: $spectrum.offset().top,  len: $spectrum.height() },
        z: { pos: $z.offset().left, len: $z.outerWidth() },
        a: { pos: $z.offset().left, len: $z.outerWidth() }
      };
      ...
      function handleMouseMove( e ) {
        ...
        var handler = ( 'xy' == dimension  ) ? spectrumHandler : sliderHandler;
        var result = handler( e.pageX, e.pageY );

        if ( ! result ) return;

        update( { controls: result } );

      }

      function sliderHandler( pageX, pageY ) {

        var slider = controlDimensions[dimension] || false;
        if ( ! slider ) return false;

        var x = ( pageX - slider.pos );
        var offsetX = ( ( x > 0 ) ? ( ( x > slider.len ) ? slider.len : x ) : 0 );

        var update = {};
        var precise = {};

        update[dimension] = offsetX / slider.len;
        precise[dimension] = offsetX;

        return { update: update, precise: precise };

      }

      function spectrumHandler( pageX, pageY ) {

        var x = ( pageX - controlDimensions.x.pos );
        var y = ( pageY - controlDimensions.y.pos );

        // Constrain to box dimensions
        x = ( ( x > 0 ) ? ( ( x >= controlDimensions.x.len ) ? controlDimensions.x.len : x ) : 0 );
        y = ( ( y > 0 ) ? ( ( y >= controlDimensions.y.len ) ? controlDimensions.y.len : y ) : 0 );

        return {
          update: { x: x / controlDimensions.x.len, y: y / controlDimensions.y.len },
          precise: { x: x, y: y }
        };
    ...
    function isValid() {
      ...
      if ( ! $el.is( 'input[type=text]' ) ) {
        console.warn( 'huebert can only be applied to text input elements.' );
        return false;
      }
    ...
    function destroy() {
      $wrapper.after( $el );
      $wrapper.remove();
      $el.removeClass( 'huebert-original-input' );
    }

    function getHueWheel( start, a ) {

      start = start || 0;

      var base = tinycolor( 'hsl ' + start + ' 1.0 0.5' );

      if ( a && a < 1 && a > 0 ) base.setAlpha( a );

      var colors = [];

      for ( var i = 0; i < 12; i++ ) {
        colors.push( base.spin( -30 ).toRgbString() );
      };

      return colors;
    }

    function linearGradient( colors, vert ) {

      // ALT return '-webkit-linear-gradient(left, #ff0000 0%, #ffff00 17%, #00ff00 33%, #00ffff 50%, #0000ff 67%, #ff00ff 83%, #ff0000 100%);';
      var colorString = colors.join( ', ' );
      var dir = ( vert ) ? 'top' : 'right';
      return 'linear-gradient( to ' + dir + ', ' + colorString + ' ); ';
    }
  ...
  $.fn.huebert = function() {
    ...
    return this.each(function() {
      ...
      var $el = $( this );
      var instance = $el.data( 'huebert-instance' );
      ...
      if ( instance ) {
        ...
        instance = new Huebert.ColorPicker( $el, options );
        $el.data( 'huebert-instance', instance );
      }
  ...
  Huebert.localize = function( i18n ) {
    for ( var item in i18n ) {
      Huebert.i18n[item] = i18n[item];
    }
  };
