...
class Essential_Grid_Facebook {
	...
	*/
	...
	public function get_photo_set_photos($photo_set_id,$item_count=10,$app_id,$app_secret){
		$oauth = wp_remote_fopen("https://graph.facebook.com/oauth/access_token?type=client_cred&client_id=".$app_id."&client_secret=".$app_secret);
		$this->oauth = $oauth;
		$url = "https://graph.facebook.com/".$photo_set_id."/photos?fields=photos&".$oauth."&fields=id,from,message,picture,full_picture,link,name,icon,privacy,type,status_type,object_id,application,created_time,updated_time,is_hidden,is_expired,comments.limit(1).summary(true),likes.limit(1).summary(true)";
	...
	/**
	 * Prepare output array $stream from feed
	 *
	 * @since    3.0
...
if(version_compare(phpversion(), '5.3', '>=') || (isset($esg_force_restapi) && $esg_force_restapi === true)){
	include 'assets/twitter/RestApi.php';
}
...
class Essential_Grid_Twitter {
  ...
  private function twitter_output_array($tweets,$count,$imageonly){
	if(is_array($tweets)){
	      ...
	      $stream['post-link'] = 'https://twitter.com/'.$this->twitter_account.'/status/'.$tweet->id;
