...
$meta_box_form = array(
  ...
  'fields'      => array(
    array(
      'name'    => __( 'Email List/Form/Campaign', '__x__' ),
      'desc'    => __( 'Choose a list/form/campaign where subscribers from this form should be added. (MailChimp uses "lists", ConvertKit uses "Forms" and GetResponse uses "Campaigns" to group subscriptions)', '__x__' ),
      'id'      => $id . '_list',
      'type'    => 'select',
      'std'     => 'Select a List',
...
if ( function_exists( 'x_add_meta_box' ) ) {
  ...
  x_add_meta_box_mailchimp();
  x_add_meta_box_custom_field();
  ...
  add_meta_box( 'email-forms-sidebar', __( 'After Publication', '__x__' ), 'email_forms_sidebar_meta_box', 'email-forms', 'side', 'default' );
} else {
