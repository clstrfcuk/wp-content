...
<form method="post" id="x-subscribe-form-<?php echo $form_id; ?>" class="x-subscribe-form x-subscribe-form-<?php echo $form_id; ?> center-block mvn" data-x-email-confirm="<?php echo $confirmation_type; ?>" <?php echo $confirmation_data; ?> style="max-width: <?php echo $max_width; ?>; font-size: <?php echo $font_size; ?>;">
  ...
  foreach ($custom_fields as $field ) : ?>
    <fieldset>
      <?php if ( $show_labels ) : ?>
        <label for="x_subscribe_form_email">
          <span><?php echo $field['label']; ?> <?php if ( $field['required'] ) : ?><span class="required">*</span><?php endif; ?></span>
        </label>
      <?php endif; ?>
      <?php
      ...
      switch ( $type ) :
        ...
        case 'radio':
          ...
          echo ( ! $show_labels ) ? $field['label'] . '<br/>' : '';
          foreach ( $choices as $choice ) : ?>
          <input type="radio" name="x_subscribe_form[<?php echo $field['name']; ?>]" id="x_subscribe_form_<?php echo $choice; ?>" value="<?php echo $choice; ?>" <?php echo isset($field['required']) && $field['required'] ? 'required' : '' ?>> <?php echo $choice; ?><br/>
        <?php
          endforeach;
          break;
        ...
        echo ( ! $show_labels ) ? $field['label'] . '<br/>' : '';
        ?>
        <select name="x_subscribe_form[<?php echo $field['name']; ?>]" id="x_subscribe_form_<?php echo $field['name']; ?>_country" <?php echo isset($field['required']) && $field['required'] ? 'required' : '' ?>>
          <option value=">"><?php echo sprintf( __( '-- Select %s --', '__x__' ), $field['label'] ); ?></option>
          <?php
        ...
        case 'address':
          $subfields = array(
            'addr1' => __( 'Street Address', '__x__' ),
            'addr2' => __( 'Address Line 2 (optional)', '__x__' ),
            'city'  => __( 'City', '__x__' ),
            'state' => __( 'State/Prov/Region', '__x__' ),
            'zip'   => __( 'Postal/Zip', '__x__' ),
          );
          foreach ( $subfields as $subfield => $sublabel) : ?>
          <input type="text" name="x_subscribe_form[<?php echo $field['name']; ?>][<?php echo $subfield; ?>]" id="x_subscribe_form_<?php echo $field['name']; ?>_<?php echo $subfield; ?>" <?php echo isset($field['required']) && $field['required'] ? 'required' : '' ?> placeholder="<?php echo $sublabel ?>">
          <?php endforeach; ?>
          <select  name="x_subscribe_form[<?php echo $field['name']; ?>][country]" id="x_subscribe_form_<?php echo $field['name']; ?>_country" <?php echo isset($field['required']) && $field['required'] ? 'required' : '' ?>>
            <option value=">"><?php _e( '-- Select a country --', '__x__' ); ?></option>
            <?php
        ...
        case 'birthday':
      ?>
      <input type="text" name="x_subscribe_form[<?php echo $field['name']; ?>]" id="x_subscribe_form_<?php echo $field['name']; ?>" placeholder="<?php echo $field['label']; ?>"  <?php echo isset($field['required']) && $field['required'] ? 'required' : '' ?>>
      <?php
          break;
  ...
  if ( isset( $mailchimp_groups )) :
    foreach ($mailchimp_groups as $group ) :
      if ( isset($group['default']) && $group['default'] !== 'display-on-form') {
        ?>
          <input type="hidden" name="x_subscribe_form[groups][<?php echo $group['id']; ?>]" id="x_subscribe_form_group_<?php echo $group['default']; ?>" value="<?php echo $group['default']; ?>">
        <?php
        continue;
      }
      ...
      switch ( $group_type ) :
        case 'radio':
          ...
          foreach ( $group['interests'] as $interest ) :
            ?>
              <input type="radio" name="x_subscribe_form[groups][<?php echo $group['id']; ?>]" id="x_subscribe_form_group_<?php echo $interest['id']; ?>" value="<?php echo $interest['id']; ?>"> <?php echo $interest['name']; ?><br/>
            <?php
          endforeach;
          break;
        ...
        case 'checkboxes':
          ...
          foreach ( $group['interests'] as $interest ) :
            ?>
              <input type="checkbox" name="x_subscribe_form[groups][<?php echo $group['id']; ?>][]" id="x_subscribe_form_group_<?php echo $interest['id']; ?>" value="<?php echo $interest['id']; ?>"> <?php echo $interest['name']; ?><br/>
            <?php
          endforeach;
          break;
        ...
        case 'dropdown':
          echo ( ! $show_labels ) ? $group['title'] . '<br/>' : '';
          ?>
          <select name="x_subscribe_form[groups][<?php echo $group['id']; ?>]" id="x_subscribe_form_group_<?php echo $group['id']; ?>">
            <option value=""></option>
