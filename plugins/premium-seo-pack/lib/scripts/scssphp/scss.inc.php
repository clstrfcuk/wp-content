...
class scssc {
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
			...
			$code = OMB()->wp_filesystem->get_contents($path);
...
class scss_server {
	...
	protected function needsCompile($in, $out) {
		...
		if (is_readable($icache)) {
			$imports = unserialize(OMB()->wp_filesystem->get_contents($icache));
	...
	protected function compile($in, $out) {
		...
		$css = $this->scss->compile(OMB()->wp_filesystem->get_contents($in), $in);
		...
		OMB()->wp_filesystem->get_contents($out, $css);
		OMB()->wp_filesystem->get_contents($this->importsCacheName($out),
	...
	public function serve($salt = '') {
		...
		if ($input = $this->findInput()) {
			...
			echo OMB()->wp_filesystem->get_contents($output);
