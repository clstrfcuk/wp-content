...
if(isset($_GET['campaign']) ) {
  $listIDs = esc_attr( $_GET['campaign'] );
  $listArray = explode("||", $listIDs);
} else {
  $listArray = array('all');
}

$sDate = ( isset($_GET['sd']) && !empty($_GET['sd']) ) ? esc_attr( $_GET['sd'] ) : $cp_analytics_start_time;
$eDate = ( isset($_GET['ed']) && !empty($_GET['ed']) ) ? esc_attr( $_GET['ed'] ) : $cp_analytics_end_time;
$chartType = ( isset($_GET['cType']) && !empty($_GET['cType']) ) ? esc_attr(  $_GET['cType'] ) : 'line';
