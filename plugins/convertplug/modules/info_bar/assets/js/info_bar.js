   ...
   function hideOnDevice(devices){
    ...
    jQuery(window).on( 'load', function() {
        ...
        jQuery(".cp-info-bar").each(function(t) {
            var $this       = jQuery(this),
                ...
                cookieName  = $this.data('info_bar-id'),
    ...
    jQuery(window).on( 'load', function() {
        ...
        var $this = jQuery(this),
            ...
            style               = $this.data('info_bar-style'),
            info_bar            = $this,
            delay               = $this.data("onload-delay"),
            delay               = delay*1000,
            load_on_refresh     = $this.data('load-on-refresh'),
        ...
        var display = false,
            scheduled = info_bar.isScheduled(),
            ...
            if(!cookie && delay && display && scheduled && referred ){
                setTimeout(function() {
                    if( jQuery(".ib-display").length <= 0 ) {
                        ...
                        if( !info_bar.hasClass('cp_impression_counted') ) {
    ...
    jQuery(document).scroll(function(e){
        if( ifb_scroll_to_x ){
            ...
            jQuery(".cp-ib-onload").each(function(t) {
                ...
                var temp_cookie         = "temp_"+cookieName;
                var nounce = $this.find(".cp-impress-nonce").val();
        ...
        if(ifb_scroll_after){           
                ...
                jQuery(".ib-after-post").each(function(t) {
                    var $this               = jQuery(this),
                        ...
                        style               = $this.data('info_bar-style'),
                        info_bar            = $this,
                        scrollTill          = jQuery(".cp-load-after-post").offset().top - 30,
                        hide_on_device      = $this.data('hide-on-devices'),
                        hide_from_device    = hideOnDevice(hide_on_device),
                        toggle_visible      = $this.data('toggle-visible'),
    ...
    jQuery(document).on('mouseleave', function(e){
        ...
        jQuery(".cp-ib-onload").each(function(t) {
            ...
            var display = false,
                inactive_time = $this.data('inactive-time');
        ...
        if( info_bar.hasClass("cp-pos-top")){
            ...
            if( page_push_down ) {
                var cp_top_offset_container = jQuery("#cp-top-offset-container").val(),
                    ...
                    mTop                = offset_def_settings.margin_top,
    ...
    jQuery(document).ready(function() {
        ...
        jQuery.each(jQuery('.cp-info-bar'),function(){
            ...
            if( typeof info_bar_custom_class !== "undefined" && info_bar_custom_class !== "" ){
                ...
                jQuery.each( info_bar_custom_class, function( index, classname ){
                    if( typeof classname !== "undefined" && classname !== "" ){
                        cls[classname] = info_bar_container;
                    }
                });
            }
        });
                ...
                jQuery("."+index).click(function(e){
                    ...
                    if( !target.hasClass('cp-form-submit-success') ) {
                        ...
                        var exit       = target.data("exit-intent"),
                            class_id   = target.data("custom-class"),
                            cookieName = target.data('info_bar-id'),
                            opt        = target.data('option'),
                            style      = target.data('info_bar-style'),
                            info_bar   = target,
                            nounce     = jQuery(this).find(".cp-impress-nonce").val(),
                            data       = {action:'smile_update_impressions',impression:true,style_id:style,option:opt,security:nounce},
                            page_down  = target.data('push-down'),
                            page_down  = parseInt( page_down ),
                            ib_height  = target.outerHeight();
    ...
    function update_impressions(styles) {
        ...
        var data = {action:'smile_update_impressions',impression:true,styles:styles,option:'smile_info_bar_styles',security:nounce};
    ...
    function cal_top_margin_push_down(info_bar,animate_push_page, toggle ) {
        ...
        var push_down_top = (ib_height + site_offset) - wpadminbar,
            ...
            cp_push_down_support_ht = jQuery(cp_push_down_support_container).outerHeight(), // Calculate height of user entered fixed class / ID
    ...
    function apply_push_page_down(info_bar) {
        ...
        setTimeout(function() {
            ...
            var has_toggle_btn  = info_bar.data('toggle'),
                toggle_visible = info_bar.data('toggle-visible') || null,
                toggle = false;
    ...
    function count_inline_impressions()  {
        jQuery(".cp-info_bar-inline-end").each(function(e) {
            ...
            if( is_visible ) {
                var styleArray = Array();
                if( !jQuery("[data-info_bar-style="+style_id+"]").hasClass('cp_impression_counted') ) {
...
function cp_infobar_social_responsive(){
    ...
    var wh = jQuery(window).width();
     jQuery(".cp_social_networks").each(function() {
        var $this = jQuery(this),
            column_no = $this.data('column-no'),
            classname ='';

        if(wh < 768){
            $this.removeClass('cp_social_networks');
            $this.removeClass(column_no);
            classname =  $this.attr('class');
            $this.attr('class', 'cp_social_networks cp_social_autowidth ' + ' ' + classname );
        }else{
            $this.removeClass('cp_social_networks');
            $this.removeClass('cp_social_autowidth');
            $this.removeClass(column_no);
            classname =  $this.attr('class');
            $this.attr('class', 'cp_social_networks ' + ' ' + column_no + ' ' + classname );
        }
     });
}
 ...
 //Open infobar scroll upto particular class/id
    ...
    var ifb_scroll_to_div = true;   
    jQuery.each(ifb_scrollcls, function(i,v){
       jQuery(document).scroll(function(e){
            if( ifb_scroll_to_div ){
                    // count inline impressions
                    count_inline_impressions();
                        ...
                        if(scroll_upt == v){
                            ...
                            if( typeof scrollclass !== 'undefined' && scrollclass !== ' ' ){
                                var div_ht = jQuery(scrollclass).outerHeight(),
                                    position    = jQuery(scrollclass).position();
