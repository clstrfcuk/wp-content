jQuery(document).ready(function() {
	...
	//	Also added class '.cp-customizer-infobar' for this individual module.
	//	Also added class '.cp-THEME_NAME' to provide the theme compatibility 
	jQuery('html').addClass('cp-customizer cp-customizer-info_bar cp-' + cp_active_theme.slug );
 ...
 *-------------------------------------------------------------------*/
function cp_infobar_append_gfonts(Fonts) {
	//	Get the last character from font list
	var lastChar = Fonts.substring(Fonts.length-1);

	//	Check & Removed last character if it's pipe (|) operator
	//	to fix MacBook Google Fonts issue
	if( lastChar == "|") {
...
function cp_infobar_append_to_ckeditor(CKFonts) {
	if( typeof CKFonts != 'undefined' && CKFonts != null && CKFonts != '') {		
		CKEDITOR.config.font_names = CKFonts;
	}
}
function cp_infobar_get_gfonts(GFonts) {
	...
	if(typeof GFonts != 'undefined' && GFonts != null && GFonts != '' ) {
		...
		if(GFonts.indexOf(',') >= 0) {
			...
			//	append google fonts
			cp_infobar_append_gfonts(Fonts);

			//	Append selected google fonts to - CKEditor
			cp_infobar_append_to_ckeditor(CKFonts);
