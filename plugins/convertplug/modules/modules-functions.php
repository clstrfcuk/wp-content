...
if( !function_exists('cp_notify_error_to_admin') ) {
	function cp_notify_error_to_admin($page_url) {
		...
		$body = "Hello there, <p>There appears to be an issue with the ConvertPlug configuration on your website. Someone tried to fill out ConvertPlug form on ".esc_url( $page_url )." and regretfully, it didn't go through.</p>";
...
if( !function_exists('cp_notify_sub_to_admin') ) {
	function cp_notify_sub_to_admin( $list_name, $subscriber_data, $sub_email ) {
