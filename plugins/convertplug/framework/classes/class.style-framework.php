...
if( !function_exists('Smile_Style_Dashboard') ){
	function Smile_Style_Dashboard( $class, $option_name, $module ) {
		...
		$ge_theme = isset( $urlData['theme'] ) ? esc_attr( $urlData['theme'] ) : '';
		$style_view = isset( $urlData['style-view'] )  ? esc_attr( $urlData['style-view'] ) : '';
		...
		if( !empty($styles) ) {
			...
			if( isset( $_GET[ 'variant-style' ] ) ) {
				if( is_array ( $smile_variant_tests ) && !empty( $smile_variant_tests ) ){
					if( isset( $_GET[ 'action' ] ) && $_GET['action'] == 'cp_new' ){
				...
				} elseif( isset( $_GET[ 'action' ] ) && $_GET['action'] == 'cp_new' ){
					$prev_styles = get_option('smile_modal_styles');
					$key = search_style($prev_styles,$style_id);
					$style_settings = $prev_styles[$key];
					$style_settings = unserialize($style_settings['style_settings']);
			...
			if( $style_view == 'new' ) { // if on template list screen
			...
			foreach( $styles as $style => $options ) {
				...
				if( isset( $style_view ) && $style_view !== "variant" ){
				...
				if( isset($_GET['action']) &&  $_GET['action'] == 'cp_new' && isset($_GET['variant-style']) ) {
					$style_name = '';
				}
				...
				$data_view = ( isset( $style_view ) && $style_view == "new" ) || ( isset( $_GET['variant-test'] ) && $_GET['variant-test'] == "new" ) ? 'data-view="new" ' : 'data-view="edit"';
				...
				if( $style_view == "variant" ) {
					...
					if( isset( $_GET["style-view"] ) && $style_view !== "edit" ){
				...
				if( isset( $style_view ) && ( $style_view == "edit" || $style_view == "variant" && $_GET['variant-test'] == 'edit') ) {
				?>
			...
			function displayPopup( style, title, url, style_settings_method, temp_name ){
				...
				jQuery("head").append('<link rel="stylesheet" type="text/css" id="style_preview_css" href="#" >');
...
if( !function_exists( "live_preview_style_css" ) ){
	function live_preview_style_css($hook){
		...
		if( strpos(	$page, "convertplug" ) !== false ) {
