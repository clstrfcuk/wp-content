...
function social_media_box_scripts($hook){
	...
	if( $cp_page == 7 || isset( $_GET['view'] )){
		if( $cp_page == 7 && isset( $data['cp-dev-mode'] ) && $data['cp-dev-mode'] == '1' ){
			...
			wp_enqueue_script('social-media',plugins_url('social-media.js',__FILE__),array('jquery','cp-swal-js'),'1.0.0',true);
		}
	}
}

/**
* Function to handle new input type "socail_media"
*
* @param $settings		- settings provided when using the input type "socail_media"
...
//var_dump($_value_array);
	...
	$output .= '<div class="social-media">';
		...
		$output .= '<div class="toggle-accordion-content">';
				...
				$output .= '<select class="cp_sm_select_action" name="input_action" id="cp-input_action-'.$uniq.'">';
					...
					if(!empty($action_types)) :
						foreach ($action_types as $key => $actiontype) :
							...
							$output .= '<option value="'.$str_lower.'" '.$selected.'>'.ucfirst($actiontype).'</option>';
							if($current_input_action_val === $actiontype && $actiontype === 'profile_link') {
								$is_profile = true;
							}
							else if($current_input_action_val === $actiontype && $actiontype === 'social_sharing') {
								$is_share = true;
							}
			...
			//share url
			$output .= '<div class="social-media-field">';
				$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="Enter the custom URL that you would like to share." style="cursor: help;float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
			...
			$output .= '<div class="social-media-field">';
				//$output .= '<label>'.__('Share Count', 'smile').'</label>';
...
//add_action( 'wp_ajax_repeat_social_media_count', 'repeat_social_media_callback_count' );
