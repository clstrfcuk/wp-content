...
class Import {
	...
	public function import_gallery() {
		...
		$gallery_post = array(
		  'ID'           => $post_id,
		  'post_title'   => sprintf( __( 'Imported Gallery #%s', 'envira-gallery' ), $post_id ),
		  'post_content' => 'imported-gallery-' . $post_id,
		);
	...
	public function import_gallery_item( $id, $item, $data, $post_id ) {
		...
		if ( ! $image ) {
			...
			if ( empty( $item['src'] ) && empty( $item['link'] ) ) {
				$this->errors[] = __( 'No valid URL or link found for the image ID #' . $id . '.', 'envira-gallery' );
	...
	public function notices() {
		...
		if ( empty( $this->errors ) && isset( $_GET['envira-gallery-imported'] ) && $_GET['envira-gallery-imported'] ) :
		?>
