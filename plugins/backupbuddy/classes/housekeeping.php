...
class backupbuddy_housekeeping {
	...
	public static function purge_logs( $file_prefix = 'status-', $limit = '', $max_age = '' ) {
		...
		$files = pb_backupbuddy::$filesystem->glob_by_date( backupbuddy_core::getLogDirectory() . $file_prefix . '*.txt', $mode = 'mtime' );
		if ( ! is_array( $files ) ) {
			$files = array();
		}
	...
	public static function trim_remote_send_stats( $file_prefix = 'send-', $limit = '', $max_age = '' ) {
		...
		// Gets files, newest first.
		$send_fileoptions = pb_backupbuddy::$filesystem->glob_by_date( backupbuddy_core::getLogDirectory() . 'fileoptions/' . $file_prefix . '*.txt', $mode = 'mtime' );
